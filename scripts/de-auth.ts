...
async function checkStatus(): Promise<void> {
  ...
  try {
    ...
    if (!data.data.configured) {
      ...
      log('cyan', '  npm run de-auth:refresh');
      return;
    }
...
async function deployOnly(): Promise<void> {
  ...
  if (!fs.existsSync(credentialsPath)) {
    ...
    log('yellow', '\nRun "npm run de-auth:refresh" to authenticate first.');
    return;
  }
...
${colors.cyan}Examples:${colors.reset}
  ...
  ${colors.yellow}npm run de-auth:status${colors.reset}

  # Perform full refresh flow (browser login + deploy)
  ${colors.yellow}npm run de-auth:refresh${colors.reset}

  # Deploy existing local credentials
  ${colors.yellow}npm run de-auth:deploy${colors.reset}
